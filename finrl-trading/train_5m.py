...
DATA=ROOT/'live_data'; MODEL=ROOT/'live_model'; STATE=ROOT/'live_state'
DATA.mkdir(exist_ok=True); MODEL.mkdir(exist_ok=True); STATE.mkdir(exist_ok=True)
logging.basicConfig(level=logging.INFO,format='%(asctime)s [5m] %(message)s')
...
def klines(): return pd.DataFrame(
    http.get(f"{BASE}/futures/usdt/candlesticks",params={"contract":"ETH_USDT","interval":"5m","limit":500},timeout=10).json()
...
def shared():
    ...
    fr_h=fr['r'].astype(float).resample('5min').ffill()
    ...
    try:
        ...
        if 'asks' in ob and 'bids' in ob:
            a=float(ob['asks'][0][0]) if ob['asks'] else 0; b=float(ob['bids'][0][0]) if ob['bids'] else 0
...
def build(df,s):
    ...
    df['price_position']=((df['close']-df['low'].rolling(20).min())/(df['high'].rolling(20).max()-
        df['low'].rolling(20).min()+1e-8))
...
def main():
    ...
    while True:
        try:
            ...
            bk=MODEL/f'ppo_5m_{ts}.pth'
            if not bk.exists(): torch.save(ag.act.state_dict(),str(bk))
            eq=round(info['eq'],2);dir_=info['d']
            price=round(float(df['close'].iloc[-1]),2)
            # 写状态
            import json
            with open(STATE/'5m.json','w') as f:json.dump({'time':time.strftime('%H:%M'),'price':price,'equity':eq,'direction':dir_,'loss':round(float(cl),2) if 'cl' in dir() else 0},f)
