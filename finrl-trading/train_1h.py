...
logging.basicConfig(level=logging.INFO,format='%(asctime)s [1h] %(message)s')
...
def klines(): return pd.DataFrame(
    http.get(f"{BASE}/futures/usdt/candlesticks",params={"contract":"ETH_USDT","interval":"1h","limit":500},timeout=10).json()
...
def shared():
    fr=pd.DataFrame(http.get(f"{BASE}/futures/usdt/funding_rate",params={"contract":"ETH_USDT","limit":500},timeout=10).json())
    fr['t']=pd.to_datetime(fr['t'].astype(int),unit='s'); fr.set_index('t',inplace=True); fr=fr.iloc[::-1]
    fr_h=fr['r'].astype(float).resample('1h').ffill()
...
def main():
    ...
    while True:
        try:
            ...
            bk=MODEL/f'ppo_1h_{ts}.pth'
            if not bk.exists(): torch.save(ag.act.state_dict(),str(bk))
            eq=round(info['eq'],2);dir_=info['d']
            price=round(float(df['close'].iloc[-1]),2)
            import json
            with open(STATE/'1h.json','w') as f:json.dump({'time':time.strftime('%H:%M'),'price':price,'equity':eq,'direction':dir_,'loss':round(float(cl),2) if 'cl' in dir() else 0},f)
